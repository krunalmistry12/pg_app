...
export default function GenerateRentPage() {
  ...
  const fetchInitialData = async (isFirstLoad = false) => {
    try {
      ...
      const list: TenantItem[] = tenantArray.map((item: any) => ({
        ...
        phone: item.phone || item.mobileNumber || item.phoneNumber || "",
        email: item.email || "",
  ...
  const sendRentNotificationToTenant = async (
    ...
    const message = `Hello ${tenant.name}, your rent bill of ₹${amount.toLocaleString()} for ${selectedMonth} ${selectedYear} has been generated by the PG Manager. Please pay on time.`;
  ...
  const handleGenerateRent = async () => {
    ...
    try {
      ...
      const promises = selectedTenantObjects.map((tenant) =>
        generateMonthlyRent(Number(tenant.id), monthIndex, yearNum),
      );
      ...
      Alert.alert(
        "Success",
        "Tenant monthly rent dues added & notifications sent!",
      );
      fetchMonthData();
    } catch (error: any) {
      console.log(
            ...
            {submitting ? (
              ...
              <View style={styles.btnContent}>
                ...
                <Text style={styles.generateButtonText}>
                  Generate Rent & Notify ({selectedTenantIds.length} Selected)
                </Text>
              </View>
            )}
          </TouchableOpacity>
